...
// This is ugly, but waaay easier than other solutions I find right now
...
datelist
{
	...
	bool mday[31];
	bool month[12];
	//int year[115]; // TODO CONSIDER you can define at max 100 different years here. Not perfect.
...
needs
{
	...
	time_t earliest; // earliest time to start with sessions
	time_t latest; // point, where everything should be done. This can be a date, but can also specify a time.
	// defines the max and min minutes spent per day/week/month/year
	uint max_per_day;
	uint max_per_week;
	uint max_per_month;
	uint max_per_year;
	...
	uint preferred_len;
	datelist *preferred;
	// TODO This is advanded stuff, not for the beginning
	// uint min_dist;
	// uint max_dist;
	// uint pref_dist;
...
init_datelist(datelist *d)
{
	for(int i=0; i < 100; i++){
		//d->year[i] = 0;
		if(i < 60) d->minute[i] = false;
		if(i < 24) d->hour[i] = false;
		if(i < 7) d->wday[i] = false;
		if(i < 31) d->mday[i] = false;
		if(i < 12) d->month[i] = false;
	}
}
...
chunk *
calculate_sessions_based_on_chunks(needs n, chunk *chunks_og, int chunks_len, int *sessions_len)
{
	// We try to respect all preferences. We will disable them one by one later on if needed.
	...
	*sessions_len = 0;
	...
	while(PREF_PER_ENABLED || PREF_DIST_ENABLED || PREF_LEN_ENABLED){
		...
		memcpy(chunks, chunks_og, chunks_len * sizeof(chunk));
		int i = 0;
		int totallen = 0;
		*sessions_len = 0;
		...
		for(i=0; i < chunks_len && totallen < n.length; i++){
			// we know that every chunk is at least n.session_len_min long and therefore we write the start here already.
		...
		// This is still a big problem! We don't pass the zones(the function doesnt know..), I don't know how to calculate this
		...
		while(totallen > n.length){
			...
			if(totallen-chunk_len(sessions[worst_i])/60 >= n.length){
				...
				//TODO "highly" inefficient, but i don't have a good way of getting the second biggest right now.
		...
		for(int j=0; j < *sessions_len; j++){
			for(int k=0; k < chunks_len; k++){
				...
				if(sessions[j].start > chunks_og[k].end)
					continue;
				if(sessions[j].start < chunks_og[k].start)
					break;
				...
				// therefore we can just take the minimum, aka the beginning of the chunk
				int diff;
				if(j == 0)
					diff = sessions[j].start - chunks_og[k].start;
				else {
					diff = sessions[j].start - (sessions[j-1].end);
					if(chunks_og[k].start > sessions[j-1].end)
						diff = sessions[j].start - chunks_og[k].start;
				}
...
calculate_chunks(needs n, chunk **chunk_00, int *chunk_00_index, chunk **chunk_02, int *chunk_02_index)
{
	...
	bool chunk_00_active = false;
	bool chunk_02_active = false;
...
{
	// Every option is evaluated between 0 and 1. Afterwards weighed and added
...
time_pref(needs n, time_t t)
{
	...
	// We start with 2, as if the time is preferred its better, so we have to subtract 2. we don't want values under 0 i think
	int pref = 2;
...
time_is_in_datelist(datelist d, icaltimetype t)
{
    ...
    // if all if's don't return false, then everything matches (the time is in the datelist!)
    return begin;
}
...
needs_to_string(needs n, char* out, int out_len)
{
	...
	// Then we store the disallowed and preferred array (+ their size), which sizes can vary
...
main(void)
{
	...
	if(!event_new(n, &event_len)){
		printf("Couldn't find a time for you. Sorry!\n");
		exit(1);
	}
